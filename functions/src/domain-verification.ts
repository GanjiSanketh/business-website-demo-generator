...
import { requireAuth, requireBusinessOwner, CallableRequest } from './auth';
...
function isValidDomainHostname(domain: string): boolean {
...
async function queryVerificationTxtRecord(
  domain: string,
...
export async function verifyCustomDomain(
  request: CallableRequest<VerifyCustomDomainRequest>
  ...
  if (!businessId || !domain) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'businessId and domain are required'
    );
  }
  ...
  if (customDomain.domain !== normalizedDomain) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Domain does not match configured domain'
    );
  }
  ...
  if (customDomain.status !== 'pending') {
    ...
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Domain is not in pending state'
    );
  }
  ...
  if (!verificationToken) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'No verification token available. Reconnect the domain first.'
    );
  }

  const hasConflict = await checkVerifiedDomainConflict(
    db,
    normalizedDomain,
    businessId
  );
  if (hasConflict) {
    throw new functions.https.HttpsError(
      'already-exists',
      'This domain is already verified for another business'
    );
  }
